...
* Porque é que não foi preciso instanciar `crazyVar` antes de usarmos o
  indexador (linha 2)? Teria sido necessário instanciar `crazyVar` se o tipo
  `Crazy` fosse uma classe?
* Na sequência do código apresentado, o que seria impresso no ecrã pela
  seguinte linha de código: `Console.WriteLine(crazyVar[100]);`
...
interface? Como poderias resolver o problema (ao nível da interface)?
...
variancia em genericos
meter varias interfaces e classes q derivam delas e perguntar se certo
codigo é possivel ou nao
-->
