...
ACEs podem ter novas gerações, que dependem da geração atual e da regra de
computação da nova geração. Esta regra define como cada célula da nova geração
é criada a partir das suas progenitoras. Mais especificamente, cada célula da
nova geração depende da sua progenitora direta bem como das suas duas vizinhas.
E como sabemos o estado das vizinhas nas extremidades do _array_? Não há
problema, pois podemos considerar o mundo toroidal. Por exemplo, o vizinho
esquerdo da célula 0 é a célula 4 (num _array_ de tamanho 5).
...
exemplo a [regra 110](https://en.wikipedia.org/wiki/Rule_110) corresponde ao
valor binário 01101110, funcionando da seguinte forma para determinar as
gerações seguintes:

| Progenitor e vizinhos         | 111 | 110 | 101 | 100 | 011 | 010 | 001 | 000 |
|-------------------------------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Novo estado da célula central |  0  |  1  |  1  |  0  |  1  |  1  |  1  |  0  |

Para o exemplo apresentado, `.*..*`, a próxima geração será `**.**`. A análise
célula a célula é a seguinte:
...
37 - Estás a desenvolver um jogo em que os passos seguidos por um personagem
para realizar um ataque são sempre os mesmos, dependendo da arma, nomeadamente:
...
38 - Resolve o problema anterior usando _delegates_. Em que condições é
     possível usar _delegates_ para este efeito e qual a vantagem de os usar
     neste caso.
