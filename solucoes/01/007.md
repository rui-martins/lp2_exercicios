...
1. Os conteúdos do ficheiro `.gitignore` devem ser aqueles que pretendemos ignorar. Neste caso são: `*.xyzconfig`, `*.xyzproj` e `*.xyzlocaldefs`.

2.Os conteúdos do ficheiro `.gitattributes` devem ser aqueles que pretendemos incluir. Neste caso são: `*.xyzsettings`, `*.png`, `*.psd`,
`*.wav` e `*.3dx`. Para gerar estes conteúdos tenho que recorrer ao comando `git lfs track` adicionando, depois de _track_, a extensão (entre parênteses), por exemplo `git lfs track "*.png"`.
