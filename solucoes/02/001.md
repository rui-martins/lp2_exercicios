 ...
 *Nota*: Esta solução usa funções locais (funções dentro de um método, só disponíveis para esse método), que foram introduzidas no C# 7.0, pelo que ainda não são muito comuns. Neste caso o método `Main()` tem duas funções locais, `IsNumeric()` e `IsFloat()`.
